...
def print_episode(i_episode, reward, n_comb, n_states, str_config):
   if i_episode%10==0:
      print("episode ", i_episode, ": reward: ", reward, "\t(", reward*(n_comb+n_states), "/", (n_comb+n_states), ")")
...
def print_stats(dut, n_states, best_reward, best_episode_states):
   ...
   for i in range(n_states):
      for j in range(n_states):
         if dut.comb_covered[i][j] != '-': 
            stats_f.write("dut.comb_covered[%s"   % repr(i))
            stats_f.write("][%s"   % repr(j))
            stats_f.write("] = %s\t"   % repr(dut.comb_covered[i][j])   )
      stats_f.write("\n")
